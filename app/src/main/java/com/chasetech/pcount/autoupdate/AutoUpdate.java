...
public class AutoUpdate {
    ...
    private ArrayList<AsyncTask<Void, String, Boolean>> asyncTaskArrayList;
    ...
    final static String API_URL_APK = "http://ulp-projectsos.com/api/check";
    final static String API_URL_APK_BETA = "http://ulp-projectsos.com/api/betacheck";
    final static String DAVIES_APK_BETA = "http://daviespcount.chasetech.com/api/betacheck";
    ...
    public final static String DAVIES_BETACHECK = "http://daviespcount.chasetech.com/api/verify";
    ...
    public AutoUpdate(Context ctx) {
        this.mContext = ctx;
        asyncTaskArrayList = new ArrayList<AsyncTask<Void, String, Boolean>>();
        SetUpVars(ctx);
...
//        mNotifyManager.notify(NOTIF_ID, mBuilder.build());
        ...
        asyncTaskArrayList.add(checkUpdateTask);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if (status == DownloadManager.STATUS_SUCCESSFUL) {
                    ...
                    MainLibrary.messageBox(mContext, "Download " + strStatus, "Download status: " + strStatus + "\nReason: " + strReason);
                }
            }
            catch (Exception ex) {
                String err = "Download failed. Please try to log again.";
                String exceptionErr = ex.getMessage() != null ? ex.getMessage() : err;
                MainLibrary.errorLog.appendLog(exceptionErr, TAG);
            }
        }
    };

    public void killTasks() {
        if (null != asyncTaskArrayList & asyncTaskArrayList.size() > 0) {
            for (AsyncTask<Void, String, Boolean> a : asyncTaskArrayList) {
                if (a != null) {
...
//            }
    ...
    private void DownloadApkUpdate(String url, String strFilename) throws IOException {
        ...
        request.setTitle("Downloading Pcount APK Update.");
